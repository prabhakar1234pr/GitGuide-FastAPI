...
async def initialize_day0_content(
    ...
    try:
        ...
        if not project_response.data or len(project_response.data) == 0:
            raise HTTPException(
                status_code=404, detail="Project not found or you don't have permission"
            )
        ...
        if not day0_id:
            ...
            if not day0_insert_response.data:
                raise HTTPException(
                    status_code=500, detail="Failed to insert Day 0 into roadmap_days"
                )
        ...
        for concept in day0_concepts:
            ...
            if concept.get("tasks"):
                ...
                total_tasks += len(tasks_to_insert)
                logger.debug(
                    f"   Inserted {len(tasks_to_insert)} tasks for concept {concept['title']}"
                )
